...
export class ApplicationFormComponent implements OnInit {
  ...
  async handleSubmit() {
    try {
      const torUrl = await this.uploadFile(this.formData.tor, 'tor');
      const photoUrl = await this.uploadFile(this.formData.photo, 'photo');

      if (torUrl) this.formData.tor = torUrl;
      if (photoUrl) this.formData.photo = photoUrl;
      ...
      alert('Error handling submit. Please try again.');
    }
  }

  async uploadFile(file: File, field: string): Promise<string | null> {
    if (!file) return null;
    ...
    if (error) {
      ...
      alert(`Error uploading ${field}. Please try again.`);
      return null;
    }

    return `https://hvqvmxakmursjidtfmdj.supabase.co/storage/v1/object/public/${field}/${data.path}`;
  }
